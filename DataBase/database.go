...
// Аню говорит что разные компании в зависимоти от условий используют два спозоба обращения к бд.
// dbal(database a layer)
// orm/odm
// Или напрямую(наверное что-то вроде db.Exec("SELECT INTO %s VALUES %s"))
func main() {
...
>>>>>>> 4be6b3bd90230bab04ce98fc1eba777409234449
	...
	//Проблема этой строчки: Если она используется в пакете, в методе после открытия бд, то при завершении метода она автоматически закроется.
	defer db.Close()
